...
import { 
  TrendingUp, Users, Globe2, AlertTriangle, FileText, RefreshCw, Activity 
} from 'lucide-react';
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-900">{formatNumber(stats.total_diagnoses)}</h3>
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-900">{stats.active_outbreaks}</h3>
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <p className="text-sm font-medium text-gray-500">Farmers Reached</p>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <p className="text-sm font-medium text-gray-500">Languages Served</p>
                <h3 className="text-2xl font-bold text-gray-900">{stats.languages_served}</h3>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                <h3 className="text-lg font-bold text-gray-900">Recent Disease Outbreaks</h3>
              </div>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {outbreaks.map(ob => (
                      <tr key={ob.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 font-medium text-gray-900">{ob.disease}</td>
                        <td className="px-6 py-4 text-gray-600">{ob.region}</td>
                        <td className="px-6 py-4">
                          <span className={cn("px-2.5 py-1 rounded-full text-xs font-semibold", getSeverityColor(ob.severity))}>
                            {ob.severity}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-gray-600">{formatNumber(ob.reports_count)}</td>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              <h3 className="text-lg font-bold text-gray-900 mb-6">Regional Crop Health (NDVI)</h3>
                  ...
                  <BarChart data={healthData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                    ...
                    <Bar dataKey="ndvi_score" radius={[4, 4, 0, 0]}>
                      {healthData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={entry.ndvi_score > 0.6 ? '#22c55e' : entry.ndvi_score > 0.4 ? '#eab308' : '#ef4444'} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
